...
for i in range(dir_index):
    path_to_dcvdt_file = os.path.join(main_dir_prefix, 'dcvdt-data', 'dcvdt-2d-{0}-{1}-to-{2}.dat'.format(i+1, fps*i+1, fps*(i+1)))
...
dcvdtr_all = dcvdtr_all.reshape(fps*dir_index, 1)
dcvdtcn_all = dcvdtcn_all.reshape(fps*dir_index, 1)
...
coordfile_dir_name = 'coord-{0}-{1}steps-per-dir'.format(dir_index, fps)
coordfile_dir = os.path.join(coordfile_dir_base, coordfile_dir_name)

t1 = time.time()
zdcvdt_thisindex = zdcvdt_matrix_interface(dir_index, fps, stride, bytearray(coordfile_dir, 'utf-8'), natoms, li_index, cl_index, o_indices, 
        nwater, n, m, r0_au, L_au, inv_mass_matrix, dcvdt_all)
t2 = time.time()

# OUTPUT DATA TO FILE
zdcvdt_dir = os.path.join(os.getcwd(), 'zdcvdt-data')
zdcvdt_fname = 'zdcvdt-2d-{0}-{1}-to-{2}.dat'.format(dir_index, fps*(dir_index-1)+1, fps*dir_index)
